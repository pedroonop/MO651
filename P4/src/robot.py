...
class Robot():
	...
	def start_sensors(self):
		...
		sensor_name=[]
		for i in range(0,16):
			sensor_name.append("Pioneer_p3dx_ultrasonicSensor" + str(i+1) + self.id)
...
#			print ("\033[92m Laser connected.")
	...
	def start_motors(self):
		...
		res, left_handle = vrep.simxGetObjectHandle(self.clientID, "Pioneer_p3dx_leftMotor" + self.id, vrep.simx_opmode_oneshot_wait)
		if(res != vrep.simx_return_ok):
			print("\033[93m Left motor not connected.")
		else:
			print("\033[92m Left motor connected.")

		res, right_handle = vrep.simxGetObjectHandle(self.clientID, "Pioneer_p3dx_rightMotor" + self.id, vrep.simx_opmode_oneshot_wait)
		if(res != vrep.simx_return_ok):
	...
	def start_robot(self):
		...
		"""
		res, robot_handle = vrep.simxGetObjectHandle(self.clientID, "Pioneer_p3dx" + self.id, vrep.simx_opmode_oneshot_wait)
		if(res != vrep.simx_return_ok):
	...
	def read_ultrassonic_sensors(self):
		...
		"""
		distances = []
		noDetectionDist = 10.0 #Here we define the maximum distance as 10 meters
	...
	def get_joint_position(self, side):
		"""
			Retrieves the intrinsic position of a joint.
		"""
		res, encoder =  vrep.simxGetJointPosition(self.clientID, self.motors_handle[side], vrep.simx_opmode_streaming)
		while(res != vrep.simx_return_ok):
			res, encoder = vrep.simxGetJointPosition(self.clientID, self.motors_handle[side], vrep.simx_opmode_streaming)
